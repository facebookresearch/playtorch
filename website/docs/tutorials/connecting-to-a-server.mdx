...
This tutorial uses the server we created in [the GPT server model tutorial](/docs/tutorials/server-model). You will need to have completed that tutorial for this one to work as is.
...
You can find the [completed versions of the source code](https://github.com/facebookexperimental/pytorch-live/tree/main/examples/gpt3-server-tutorials/GPTClient) we've written in this tutorial in the `examples` folder Pytorch Live GitHub repo.
