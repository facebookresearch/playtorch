...
<div className="tutorial-oneliner">In this tutorial, we will connect our PlayTorch demo to the server we created in [the GPT server model tutorial](/docs/tutorials/server-model).</div>

You will need to have completed that tutorial for this one to work as is. If you haven't installed the PlayTorch CLI yet, please [follow this tutorial](./get-started.mdx) to get started.
...
We will start by creating a new React Native project with the PlayTorch template using the CLI. Run the following command:
...
The aim of this tutorial is to help you integrate your PlayTorch apps with server models, so we will not spend much time on the UI. We will add some handling for interactions, but won't discuss layout or styling.
...
You can find the [completed versions of the source code](https://github.com/facebookresearch/playtorch/tree/main/examples/gpt3-server-tutorials/GPTClient) we've written in this tutorial in the `examples` folder PlayTorch GitHub repo.
