...
PyTorch Live's command line tool `torchlive-cli` will automate most of the setup process. Here's a sneak peek of what the process looks like:

<DocVideo asset="install" />
...
- **Our beta release only supports MacOS**. Windows and Linux supports are coming soon. However, you can [follow the manual installation instructions to get started on Windows and Linux](get-started-manually).
...
Check out the [manual installation process](get-started-manually) if your operating system is Windows or Linux or if the PyTorch Live command line tool `torchlive-cli` fails for you!
...
If everything goes well, the terminal will looks similar to the following output. If you run into any issues, please check the [FAQ](faq).
...
Start by trying out the various **Examples** in the app. When you are ready to experiment, take a look at the **Toolbox** demos and follow along with a tutorial: [recognizing objects in photographs](image-classification) or [answering questions about some text](question-answering).
