...
<div className="tutorial-oneliner">In this tutorial, you will integrate an on-device NLP (Natural Language Processing) model that can answer questions about a short paragraph of text.</div>

If you haven't installed the PlayTorch CLI yet, please [follow this tutorial](./get-started.mdx) to get started.
...
Let's start by initializing a new project `QuestionAnsweringTutorial` with the PlayTorch CLI.
...
The `run-android` and `run-ios` commands from the PlayTorch CLI allow you to run the question answering project in the Android emulator or iOS Simulator.
...
It will use the `bert_qa.ptl` model that is already prepared for PlayTorch. You can follow the [Prepare Custom Model](./prepare-custom-model.mdx) tutorial to prepare your own NLP model and use this model instead for question answering.
