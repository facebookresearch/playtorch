...
<div className="tutorial-oneliner">In this tutorial we will use a model trained on the MNIST dataset of handwritten digits to predict the number that the user draws.</div>
...
We will start by creating a new React Native project with the PlayTorch (PTL) template using the CLI. Run the following command:
...
### The PlayTorch Canvas Component
