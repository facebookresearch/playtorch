...
cast_to_int32_ops = ["argmin", "argmax"]
...
def gen_cpp_func(op: OpInfo):
    ...
    arguments_string = "\n".join(
        [
            get_argument_string(op.arguments[i], i, op.options_index)
            for i in range(len(op.arguments))
        ]
    )
    ...
    returns_string = (
        get_returns_string(op, op.name in cast_to_int32_ops) if op.implemented else ""
    )
