...
You can find the [completed versions of the source code](https://github.com/facebookexperimental/pytorch-live/tree/main/examples/gpt3-server-tutorials/ptl-server) we've written in this tutorial in the `examples` folder Pytorch Live GitHub repo.

## Next steps

Want to see a model with more complex input like images? Check out our tutorial with VQGAN + CLIP to generate images from text descriptions.

Want see how to connect to this server from an app? Check out our tutorial for connecting to our server from a React Native app.
