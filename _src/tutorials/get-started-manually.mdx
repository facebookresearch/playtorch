...
<div className="tutorial-oneliner">In this tutorial, you will manually set up the build environment, create your first PlayTorch project, and run it in an emulator or on your phone.</div>

If you ended up here, your operating system is either Windows or Linux, or the PlayTorch command line tool `torchlive-cli` failed for you. No worries, we have prepared manual steps for you to get started with PlayTorch!
...
PlayTorch uses `python3` to export PyTorch models for the PyTorch Mobile runtime. The export is part of each PlayTorch app project initialization (see later steps). Run the following command to check if python3 is installed:
...
For PlayTorch beta, we provide a React Native app template, packed with AI demos and practical examples. Use the `npx react-native init` command to initialize your first PlayTorch project.
...
Show us what you've made with PlayTorch by tagging #pytorchlive in your social media posts.
